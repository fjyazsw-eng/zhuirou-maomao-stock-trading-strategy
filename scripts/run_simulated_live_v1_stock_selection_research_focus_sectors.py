...
REALTIME_WATCH_DIR = ROOT / "reports" / "realtime_watch"
...
def apply_manual_policy_overrides(
    ...
    if "600276.SH" in selected:
        ...
        if item.get("candidate_action") == "BUY_CANDIDATE" and item.get("buy_point_quality") == "GOOD":
            item["confidence_level"] = "HIGH"
            item["trial_position_limit"] = "15%"
        else:
            item["confidence_level"] = item.get("confidence_level", "MEDIUM")
            item["trial_position_limit"] = item.get("trial_position_limit", "5%")
...
def plain_action_for(item: dict[str, Any]) -> str:
    action = str(item.get("candidate_action") or item.get("current_action") or "WATCH")
    quality = str(item.get("buy_point_quality") or "")
    if action == "BUY_CANDIDATE" and quality in {"GOOD", "FAIR"}:
        return "轻仓试错"
    if action in {"BUY_CANDIDATE", "WATCH"}:
        return "回踩再买"
...
def build_dynamic_watch_pool(
    latest_trade_date: str,
    recommended_candidates: list[dict[str, Any]],
    candidate_continuity_review: list[dict[str, Any]],
    manual_sample_stocks: list[dict[str, Any]],
    max_items: int = 10,
) -> dict[str, Any]:
    action_rank = {"BUY_CANDIDATE": 0, "WATCH": 1, "NEEDS_HUMAN_REVIEW": 2, "WATCH_ONLY": 3, "OBSERVE": 4}
    leader_rank = {"ABSOLUTE_LEADER": 0, "TREND_LEADER": 1, "CAPACITY_CORE": 2, "LOW_POSITION_REPAIR": 3, "ELASTIC_LEADER": 4}
    ...
    def add(item: dict[str, Any], source: str) -> None:
        ...
        if not ts_code:
            return
        action = str(item.get("candidate_action") or item.get("current_action") or "WATCH")
        if action not in {"BUY_CANDIDATE", "WATCH", "WATCH_ONLY", "NEEDS_HUMAN_REVIEW", "OBSERVE"}:
            return
        if item.get("leader_type") in {"AVOID", "FOLLOWER"} and action not in {"BUY_CANDIDATE", "WATCH"}:
            return
        previous = merged.get(ts_code)
        candidate = {
            "code": ts_code,
            "name": item.get("name", ""),
            "sector": item.get("sector") or item.get("linked_sector_name", ""),
            "candidate_action": action,
            "plain_action": plain_action_for({**item, "candidate_action": action}),
            "leader_type": item.get("leader_type", ""),
            "buy_point_quality": item.get("buy_point_quality", ""),
            "entry_condition": item.get("entry_condition", ""),
            "stop_loss_condition": item.get("stop_loss_condition", ""),
            "take_profit_condition": item.get("take_profit_condition", ""),
            "source": source,
            "manual_mapping_candidate": bool(item.get("manual_mapping_candidate", False)),
            "official_sector_candidate": bool(item.get("official_sector_candidate", not item.get("manual_mapping_candidate", False))),
        }
        if previous is None:
            merged[ts_code] = candidate
            return
        old_key = (
            action_rank.get(previous["candidate_action"], 9),
            leader_rank.get(previous.get("leader_type", ""), 9),
        )
        new_key = (
            action_rank.get(candidate["candidate_action"], 9),
            leader_rank.get(candidate.get("leader_type", ""), 9),
        )
        if new_key < old_key:
            merged[ts_code] = candidate

    for item in recommended_candidates:
        add(item, "recommended_candidates")
    for item in candidate_continuity_review:
        add(item, "candidate_continuity_review")
    for item in manual_sample_stocks:
        add(item, "manual_sample_stocks")

    rows = sorted(
        merged.values(),
        key=lambda item: (
            action_rank.get(item["candidate_action"], 9),
            leader_rank.get(item.get("leader_type", ""), 9),
            item["code"],
        ),
    )[:max_items]
    return {
        ...
        "boundary": "只读观察池；不接真实账户，不自动交易，不真实下单。",
        "watch_pool": rows,
    }


def write_dynamic_watch_pool_files(pool: dict[str, Any]) -> dict[str, str]:
    REALTIME_WATCH_DIR.mkdir(parents=True, exist_ok=True)
    trade_date = pool["latest_completed_trade_date"]
    dated = REALTIME_WATCH_DIR / f"dynamic_watch_pool_{trade_date}.json"
    latest = REALTIME_WATCH_DIR / "dynamic_watch_pool_latest.json"
    text = json.dumps(pool, ensure_ascii=False, indent=2)
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines += ["", "## dynamic_watch_pool"]
    for item in report.get("dynamic_watch_pool", {}).get("watch_pool", []):
        lines.append(f"- {item['name']}({item['code']}) | {item['sector']} | {item['plain_action']} | {item['candidate_action']}")
...
def main() -> int:
    ...
    dynamic_watch_pool = build_dynamic_watch_pool(
        latest_trade_date,
        recommended_candidates,
        continuity_review,
        medical_manual_section["sample_stocks"],
    )
    ...
    report = {
        ...
        "dynamic_watch_pool": dynamic_watch_pool,
    ...
    dynamic_watch_pool_paths = write_dynamic_watch_pool_files(dynamic_watch_pool)
    report["dynamic_watch_pool_paths"] = dynamic_watch_pool_paths
    json_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    md_path.write_text(render_markdown(report), encoding="utf-8")
    print(json.dumps({"json": str(json_path), "md": str(md_path), "dynamic_watch_pool": dynamic_watch_pool_paths["latest"], "latest_completed_trade_date": latest_trade_date}, ensure_ascii=False, indent=2))
    return 0
