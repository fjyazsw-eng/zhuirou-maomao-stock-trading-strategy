...
def prepare_static_caches(pro: Any | None) -> list[str]:
    ...
    for row in l2.itertuples(index=False):
        ...
        statuses.append(fetch_if_needed(pro, f"index_member_all_{index_code}", member_path(index_code), MEMBER_FIELDS, lambda p, code=index_code: fetch_sw_index_members(p, code, level="L2"), min_rows=0))
    return statuses
