    ...
    if isinstance(features, dict):
        for name in ["公司基本面", "公告新闻", "实时价格", "行业成分"]:
            ...
            if isinstance(item, dict):
                feature_parts.append(f"{name}:{item.get('status', '-')}")
    feature_text = "；".join(feature_parts) if feature_parts else "功能矩阵未检测"
    if status != "PASS":
        return f"Tushare状态：异常/待确认，优先检查接口。status={status}；latest={latest}；data={data_trade_date}；能力={feature_text}；endpoint={endpoint}；message={message}"
    return f"Tushare状态：正常。latest={latest}；data={data_trade_date}；能力={feature_text}；endpoint={endpoint}"
