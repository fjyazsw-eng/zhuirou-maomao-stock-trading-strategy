...
def main() -> int:
    ...
    parser.add_argument("--preferred-trade-date", default="20260706")
    parser.add_argument("--output-path", default=str(OUTPUT_QUOTES_PATH))
    parser.add_argument("--review-json-path", default=str(OUTPUT_REVIEW_JSON_PATH))
    parser.add_argument("--review-md-path", default=str(OUTPUT_REVIEW_MD_PATH))
    args = parser.parse_args()

    handoff_path = Path(args.handoff_path)
    output_path = Path(args.output_path)
    review_json_path = Path(args.review_json_path)
    review_md_path = Path(args.review_md_path)

    handoff_rows = load_handoff_symbols(handoff_path)
    pro = get_tushare_pro(ROOT)
    resolved_trade_date, latest_completed_trade_date, fallback = resolve_trade_date(pro, args.preferred_trade_date)
