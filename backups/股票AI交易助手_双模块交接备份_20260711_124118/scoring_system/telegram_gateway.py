...
ANSWER_CACHE_PATH = REPORT_DIR / "answer_cache.json"
...
def report_stamp() -> str:
    paths = [
        ROOT / "reports" / "latest_market_decision_report.md",
        ROOT / "reports" / "regime" / "sector_regime_20260702.json",
        ROOT / "reports" / "alerts" / "sector_alerts_20260702.json",
    ]
    stamps: list[str] = []
    for path in paths:
        if path.exists():
            stat = path.stat()
            stamps.append(f"{path.name}:{int(stat.st_mtime)}:{stat.st_size}")
    return "|".join(stamps)
...
def load_answer_cache() -> dict[str, Any]:
    if not ANSWER_CACHE_PATH.exists():
        return {"items": {}}
    try:
        payload = json.loads(ANSWER_CACHE_PATH.read_text(encoding="utf-8"))
    except Exception:
        return {"items": {}}
...
def save_answer_cache(payload: dict[str, Any]) -> None:
    REPORT_DIR.mkdir(parents=True, exist_ok=True)
    ANSWER_CACHE_PATH.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")


def cache_key(question: str, cash: float | None, llm_mode: str) -> str:
    raw = json.dumps(
        {
...
def read_cached_answer(question: str, cash: float | None, llm_mode: str, ttl_seconds: int) -> str:
    payload = load_answer_cache()
    item = payload.get("items", {}).get(cache_key(question, cash, llm_mode))
...
def write_cached_answer(question: str, cash: float | None, llm_mode: str, text: str) -> None:
    payload = load_answer_cache()
    items = payload.setdefault("items", {})
    items[cache_key(question, cash, llm_mode)] = {
        "created_at": now_ts(),
        "text": text,
    }
    if len(items) > 100:
        ordered = sorted(items.items(), key=lambda kv: int(kv[1].get("created_at", 0)), reverse=True)[:100]
        payload["items"] = dict(ordered)
    save_answer_cache(payload)
...
def compact_answer(question: str, cfg: dict[str, Any] | None = None) -> str:
    cfg = cfg or telegram_config()
    cash = extract_cash(question)
    mode = response_mode(question, cfg)
    cache_ttl = max(60, int(cfg.get("answer_cache_ttl_seconds", 900)))
    cached = read_cached_answer(question, cash, mode, cache_ttl)
    if cached:
        return cached
    ...
    if mode == "llm":
        ...
        direct = full + "\n\n说明：当前为快速模式，优先返回简洁可执行结论。"
    if mode == "llm" and llm_is_ready():
    ...
    write_cached_answer(question, cash, mode, result)
    return result
...
def response_mode(question: str, cfg: dict[str, Any]) -> str:
    forced = str(cfg.get("response_mode", "auto")).strip().lower()
    if forced in {"local", "llm", "auto"}:
        mode = forced
    else:
        mode = "auto"
    if mode != "auto":
    ...
    if len(text) >= 24:
        return "llm"
    complex_hints = ["为什么", "详细", "展开", "对比", "复盘", "逻辑", "依据", "怎么看", "分析"]
    if any(hint in text for hint in complex_hints):
        return "llm"
    return "local"


def llm_timeout_seconds(cfg: dict[str, Any]) -> int:
    raw = int(cfg.get("timeout_seconds", 14))
    return max(5, min(raw, 30))
...
def call_compatible_chat_summarizer(key: str, cfg: dict[str, Any], question: str, cash: float | None, full_answer: str) -> str:
    ...
    with urllib.request.urlopen(req, timeout=llm_timeout_seconds(cfg)) as resp:
        raw = resp.read().decode("utf-8", errors="replace")
    parsed = json.loads(raw)
...
def quick_ack_text() -> str:
    return "收到，正在整理这条问题的简版结论，稍后发回。"


def fallback_timeout_answer(question: str) -> str:
    cash = extract_cash(question)
    direct = answer(question, cash=cash)
    return "\n".join(
        [
            "股票AI交易助手",
            f"生成时间：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
            f"问题：{question}",
            "",
            clean_user_text(direct),
            "说明：本次整理超时，已降级为本地快速结论。",
            "",
            "边界：仅作辅助分析，不自动交易，不替代最终决策。",
        ]
    )
...
def total_answer_timeout_seconds(cfg: dict[str, Any]) -> int:
    raw = int(cfg.get("total_answer_timeout_seconds", 20))
    return max(6, min(raw, 45))
...
def handle_message(token: str, message: dict[str, Any], cfg: dict[str, Any], dry_run: bool = False) -> dict[str, Any]:
    ...
    elif text in {"/fast", "快答", "快速模式"}:
        reply = "快速模式说明：短问题默认走本地快速总结，复杂问题才会升级到大模型压缩。"
    else:
        question = text
        if text.startswith("问："):
            question = text[2:].strip()
        if not dry_run and response_mode(question, cfg) == "llm" and bool(cfg.get("send_ack_before_long_reply", True)):
            try:
                send_message(token, int(chat_id), quick_ack_text(), int(cfg.get("ack_max_chars", 500)))
            except Exception:
                pass
        try:
            with ThreadPoolExecutor(max_workers=1) as pool:
                future = pool.submit(compact_answer, question, cfg)
                reply = future.result(timeout=total_answer_timeout_seconds(cfg))
        except FutureTimeoutError:
            reply = fallback_timeout_answer(question)
