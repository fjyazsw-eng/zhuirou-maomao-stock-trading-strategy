...
def decide_watch_state(
    ...
    reasons: list[str] = []
    active_states = {"ZONE_REACHED", "HOLDING", "CONFIRMED", "TURNING_STRONG"}
    ...
    if stock.price < low * (1 - tolerance):
        ...
        if low <= stock.price <= high:
            ...
            state = "ABOVE_ZONE"
            reasons.append("价格在观察区上方，等待回踩确认")
        else:
            state = "BELOW_ZONE"
...
def build_message(decision: WatchDecision) -> str:
    if decision.volume_ratio is None:
        volume = "无可比数据"
    elif decision.volume_ratio_source == "tushare_5d_elapsed_projection":
        volume = f"前5日成交进度估算的{decision.volume_ratio:.2f}倍"
    else:
        volume = f"可用历史同期的{decision.volume_ratio:.2f}倍"
    funds = "资金数据缺失" if decision.main_net is None else f"主力净流入{decision.main_net / 10000:.0f}万元"
    sector = "板块数据缺失" if decision.sector_pct is None else f"板块{decision.sector_pct:+.2f}%"
    market = "大盘数据缺失" if decision.market_avg_pct is None else f"大盘均值{decision.market_avg_pct:+.2f}%"
    reason = "；".join(decision.reasons[:2])
    lines = [
            f"股票AI交易助手盯盘提醒｜{decision.name} {decision.code}",
            f"状态：{STATE_LABELS.get(decision.state, decision.state)}",
            f"时间：{decision.trade_time}　最新价：{decision.price:.2f}" if decision.price is not None else f"时间：{decision.trade_time}",
    ]
    if decision.kind == "holding" and decision.levels:
        levels = decision.levels
        pnl = ((decision.price / decision.cost - 1) * 100) if decision.price is not None and decision.cost else None
        lines.append(
            f"持仓：{decision.quantity}股　成本：{decision.cost:.2f}　浮动：{pnl:+.2f}%" if pnl is not None else f"持仓：{decision.quantity}股"
        )
        level_text = f"保护位{levels['support']:.2f}　风险线{levels['risk']:.2f}　修复位{levels['recovery']:.2f}"
        if levels.get("strength"):
            level_text += f"　转强位{levels['strength']:.2f}"
        lines.append(level_text)
    else:
        lines.append(f"观察区：{decision.zone_low:.2f}～{decision.zone_high:.2f}")
    lines.extend([
        f"成交：{volume}　资金：{funds}",
        f"环境：{market}　{sector}",
        f"判断：{reason}",
        "仅作盯盘和咨询，不代表自动交易。",
    ])
...
class RealtimeWatchMonitor:
    ...
    def run_once(self, now: datetime | None = None, send: bool = False, force_notify: bool = False) -> dict[str, Any]:
        ...
        for item, is_holding in all_items:
            ...
            notify_states = set(self.config.get("notification", {}).get("states", []))
            ...
            missing_platforms = [
                ...
                if force_notify or previous_platform_states.get(platform) != decision.state
            ]
    ...
    def run_forever(self, send: bool = True) -> None:
        ...
        while True:
            ...
            in_session = weekday and ("09:25" <= current_time <= "11:35" or "12:55" <= current_time <= "15:05")
            if in_session:
                try:
